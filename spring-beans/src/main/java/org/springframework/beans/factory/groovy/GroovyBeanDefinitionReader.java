...
/**
 * 用于 Spring bean 定义的基于 Groovy 的阅读器：类似于 Groovy 构建器，但更多的是用于 Spring 配置的 DSL。
 *
 * <p>这个 bean 定义阅读器还理解 XML bean 定义文件，允许与 Groovy bean 定义文件无缝混合和匹配。
 *
 * <p>通常应用于
 ...
 * }</pre>
 *
 * <p>您还可以使用 {@link #loadBeanDefinitions(Resource...)} 或 {@link #loadBeanDefinitions(String...)}
 * 方法加载包含在 Groovy 脚本中定义的 bean 的资源，脚本类似于以下内容。
 *
 * <pre class="code">
 * import org.hibernate.SessionFactory
 * import org.apache.commons.dbcp.BasicDataSource
...
public class GroovyBeanDefinitionReader extends AbstractBeanDefinitionReader implements GroovyObject {
	...
	/**
	 * 标准 {@code XmlBeanDefinitionReader} 使用默认设置创建，用于从 XML 文件加载 bean 定义。
	 */
	...
	/**
	 * 为给定的创建一个新的 {@code GroovyBeanDefinitionReader}
	...
	/**
	 * 从指定的 Groovy 脚本或 XML 文件加载 bean 定义。 <p>注意 {@code ".xml"} 文件将被解析为 XML 内容；
	 * 所有其他类型的资源将被解析为 Groovy 脚本。
	 *
